...
//        bool RatingFloor = false;
        ...
        public void SortField(bool init = false)
        {
            ...
            int i = 1;
            if (init)
            { //late players get a different Seed because they were late and that is how it goes
        ...
        public void InitMMS()   //RM - In Swiss we all start on the same score.
        {
        }
    ...
    public int AssignBye(int _rnd, int ByeLevel = 1)
    {
        ...
        public void UpdateTiebreaks(int rnd)
        {
            ...
            foreach (Player ap in AllPlayers)
            {
                ...
                for (int i = 0; i < rnd; i++)
                {
                    ...
                    try
                    {
                        if (ap.getParticipation(i) == true)
                        {
                            ...
                            float f = AllPlayers[lookUp[op]].getSwiss();
                ...
                if (nGame == 0)
                    _SOS = ap.getSwiss() * rnd; //used to be initMMS();
                if (nGame < rnd)
                {
                    if (nGame != 0)
                    {
